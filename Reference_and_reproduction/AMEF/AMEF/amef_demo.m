...
%     I_hazya = imread(['new/' num2str(i) '.png']);
%     I_hazyb = imread(['new/' num2str(i) 'b.png']);
    % Increase clip_range to remove more haze - at the risk of overenhancement
    clip_range = 0.010;
 
    tic
%     amef_im = amef(im2double(I_hazy), im2double(I_hazya), im2double(I_hazyb), clip_range);
    ...
    imwrite(amef_im,['.\result\', tempFileName(1:end-4), '.png',])
end
end